    ...
    <nav 
      className={`flex justify-between items-center py-6 px-6 md:px-12 fixed w-full top-0 z-50 transition-all duration-500 ${
        activeScrolled 
          ...
          : (invert ? 'bg-transparent text-earth-deep shadow-none' : 'bg-transparent text-earth-light shadow-none')
      }`}
    >
      {/* THE FIX: Changed 'text-earth-light' to 'text-earth-deep' so the logo is legible when inverted */}
      ...
      <div className="hidden md:flex gap-8 font-medium items-center">
        ...
        {pathname !== '/' && (
          <Link href="/" className="hover:text-earth-sage transition-colors duration-300">Home</Link>
        )}

        {pathname !== '/our-impact' && (
          <Link href="/our-impact" className="hover:text-earth-sage transition-colors duration-300">Our Impact</Link>
        )}

        <Link href="/products" className="hover:text-earth-sage transition-colors duration-300">Products</Link>
        <Link href="/impact" className="hover:text-earth-sage transition-colors duration-300">Blog</Link>
        
        {/* Contact Us - Dropdown Container */}
        <div className="relative group">
          <Link href="/contact" className="hover:text-earth-sage transition-colors duration-300 py-4">
            Contact Us
          </Link>
            ...
            <div 
              className={`rounded-2xl shadow-xl py-2 flex flex-col overflow-hidden transition-colors duration-500 border ${
                activeScrolled 
                  ...
                  : (invert ? 'bg-white/80 backdrop-blur-md border-earth-forest/10' : 'bg-white/10 backdrop-blur-md border-white/20')
              }`}
            >
              <Link 
                href="/faqs" 
                className={`px-5 py-3 text-sm font-semibold text-center transition-colors duration-300 ${
                  activeScrolled
                    ...
                    : (invert ? 'text-earth-deep hover:bg-earth-light' : 'text-earth-light hover:bg-white/20')
                }`}
              >
                FAQs
              </Link>
      ...
      <button 
        className={`px-6 py-2 rounded-full font-semibold transition-all duration-500 shadow-md hover:scale-105 ${
          activeScrolled
            ...
            : (invert ? 'bg-earth-deep text-earth-light hover:bg-earth-forest' : 'bg-earth-light text-earth-deep hover:bg-white')
        }`}
      >
        Enquiry
      </button>
